...
my ($logfile, $pasteroot, $host, $srvname, $port, $certfile, $keyfile, $pidfile, $seclvl, $maxpastesize);
...
if (-e $pidfile) {
  die "0x07 Error: SPaste is already running or the lockfile didn't get wiped!  If you are sure it is not running, remove $pidfile";
}
...
if (!-w $pasteroot) {
  print $tee purdydate() . " 0x0B The paste root directory is not writable! Check permissions!\n";
  exit $SIG{TERM};
}

# Verify the SSL certificate and key files are readable before binding
if (!-r $certfile) {
  print $tee purdydate() . " 0x0C The certificate file is not readable! Check permissions!\n";
  exit $SIG{TERM};
}
if (!-r $keyfile) {
  print $tee purdydate() . " 0x0D The private key file is not readable! Check permissions!\n";
  exit $SIG{TERM};
}
...
if (!isint($seclvl)) {
  print $tee purydate() . " 0x11 The security level doesn't seem to be an integer!\n";
  exit $SIG{TERM};
}

# Enforce a minimum security level to prevent trivially short IDs / collisions
if ($seclvl < 8) {
  print $tee purdydate() . " 0x0F You cannot set the seclvl lower than 8! It is insecure and could cause collisions!\n";
  exit $SIG{TERM};
}
if ($seclvl < 12) {
  print $tee purdydate() . " 0x0F Warning: Setting the security level lower than 12 is probably a bad idea... continuing anyway...\n";
}

# Validate that maxpastesize is a positive integer if provided
if (defined $maxpastesize) {
  if (!isint($maxpastesize) || $maxpastesize <= 0) {
    print $tee purdydate() . " 0x12 The maxpastesize doesn't seem to be a positive integer!\n";
    exit $SIG{TERM};
  }
...
while (1) {
  eval {
    ...
    if ($cl) {
      ...
      $th->detach()
        or print $tee purdydate() . " 0x05 Error: Thread detach request failed. $!\n";
    }
...
sub server {
  ...
  while (my $line = $cl->getline()) {
    ...
    if (defined $maxpastesize && $total_bytes > $maxpastesize) {
      close(P);
      unlink($filename);
      print $cl "0x13 Error: Paste exceeds maximum allowed size of $maxpastesize bytes!\n";
      print $tee purdydate() . " 0x13 " . $cl->peerhost . " paste too large ($total_bytes bytes), rejected.\n";
      $cl->close();
      return 0;
    }
    print P $line;
  ...
  if ($cfgf) {
    if (-e $pidfile) {
      unless ($SIG{TERM} || $SIG{INT}) {
        ...
        print $tee purdydate() . " 0x02 Error: Something unusual happened... check $logfile\n";
      }
    ...
    print STDERR "0x03 Error: Something unusual happened before config was read... exiting!\n";
  }
}
...
sub check_ssl_keypair {
  ...
  if (!$ctx) {
    print $tee purdydate() . " 0x16 Error: Could not create SSL context for keypair check.\n";
    exit $SIG{TERM};
  }

  # Attempt to load the certificate into the context
  if (!Net::SSLeay::CTX_use_certificate_file($ctx, $cert_file, Net::SSLeay::FILETYPE_PEM())) {
    my $err = Net::SSLeay::ERR_error_string(Net::SSLeay::ERR_get_error());
    print $tee purdydate() . " 0x19 Error: Could not load certificate for keypair validation: $err\n";
  ...
  if (!Net::SSLeay::CTX_use_PrivateKey_file($ctx, $key_file, Net::SSLeay::FILETYPE_PEM())) {
    my $err = Net::SSLeay::ERR_error_string(Net::SSLeay::ERR_get_error());
    print $tee purdydate() . " 0x17 Error: Private key is invalid or could not be loaded: $err\n";
    Net::SSLeay::CTX_free($ctx);
    exit $SIG{TERM};
  }
  ...
  if (!Net::SSLeay::CTX_check_private_key($ctx)) {
    my $err = Net::SSLeay::ERR_error_string(Net::SSLeay::ERR_get_error());
    print $tee purdydate() . " 0x18 Error: Private key does not match the certificate: $err\n";
    Net::SSLeay::CTX_free($ctx);
    exit $SIG{TERM};
  }

  Net::SSLeay::CTX_free($ctx);
  print $tee purdydate() . " 0x00 SSL certificate and private key pair verified successfully.\n";
}
...
sub check_ssl_cert {
  ...
  if ($@ || !$cert) {
    print $tee purdydate() . " 0x1A Error: Could not parse SSL certificate file '$file': $@\n";
    exit $SIG{TERM};
  }
  ...
  if (!defined $not_before || !defined $not_after) {
    print $tee purdydate() . " 0x0C Warning: Could not read validity dates from SSL certificate.\n";
    return;
  }
  ...
  if ($now < $not_before) {
    ...
    print $tee purdydate() . " 0x1B Error: SSL certificate is not yet valid (valid from: $valid_from). Exiting.\n";
    exit $SIG{TERM};
  }
  ...
  if ($now > $not_after) {
    ...
    print $tee purdydate() . " 0x1C Error: SSL certificate has EXPIRED (expired: $expired_on)." . " Clients will receive SSL errors. Exiting.\n";
    exit $SIG{TERM};
  }
  ...
  if ($days_left <= 30) {
    ...
    print $tee purdydate() . " 0x0C Warning: SSL certificate expires in $days_left day(s) ($expires_on)." . " Please renew soon.\n";
  } else {
    print $tee purdydate() . " 0x00 SSL certificate is valid. Expires in $days_left day(s).\n";
  }
}
